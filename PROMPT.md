# MaixCAM Master Universal Prompt

Use this prompt with any AI assistant when building, debugging, tuning, or reviewing MaixCAM / MaixPy vision recognition tasks.
...
Prefer official and current sources before relying on memory:
...
If an API, module, model format, camera option, or board feature is uncertain, search or inspect official Sipeed docs, GitHub source, examples, and issues before community posts.

If this repository is available as files, read these references only when needed:

- `dist/codex/maixcam-master/references/maixcam-vision-workflow.md` for staged task analysis, algorithm selection, screen-first tuning UI, and validation loops.
- `dist/codex/maixcam-master/references/maixpy-api-map.md` for MaixPy source/API lookup guidance and code patterns.
...
Use this staged checklist for each task:
...
- A main loop that keeps the program in the foreground, reads camera/input every frame, draws the current view, sends rate-limited output, and exits gracefully.
...
When the user writes in Chinese, answer in Chinese by default. For "科普" requests, explain the mental model first, then map it to MaixCAM / MaixPy code and validation steps. For debugging, anchor conclusions in logs, device output, sample frames, official docs, or source inspection.
